...
void *loader_alloc(uint64_t size, uint32_t align)
{
    ...
    // pointer to memory with free_memory and Round up with align the memory_chunk to be aligned properly
    uint8_t* memory_chunk = (uint8_t*)ROUND_UP((uint32_t)free_memory, align);

    // Save the current value of memory_chunk as the allocated chunk
    void* allocated_chunk = (void*)memory_chunk;

    // increase free_memory to record allocation
    free_memory = (uint8_t*)(memory_chunk + size);
...
int loader_read_kernel(uint64_t *kernel_entry_point)
{
    // Allocate memory for the ELF header with proper alignment
    struct elf64_header *elf_header = (struct elf64_header *)loader_alloc(sizeof(struct elf64_header), PAGE_SIZE);
    if (disk_io_read_segment((uint32_t)elf_header, ATA_SECTOR_SIZE, KERNEL_BASE_DISK_SECTOR) != 0) {
        terminal_printf("Failed to read the ELF header");
        return 1;
    }
    
    // Check the ELF magic
    if (elf_header->e_magic != ELF_MAGIC) {
        terminal_printf("Invalid ELF format");
        return 1;
    }
    ...
    for(struct elf64_program_header *ph = ELF64_PHEADER_FIRST(elf_header); ph < ELF64_PHEADER_LAST(elf_header);ph++){
        ...
        if (disk_io_read_segment(ph->p_va, ph->p_memsz, lba) != 0) {
            terminal_printf("Failed to read other segments");
            return 1;
        }
    }
    // Update the kernel entry point
...
uint64_t loader_detect_memory(struct bios_mmap_entry *memory_map, uint32_t cnt)
{
    ...
    for (uint32_t i = 0; i < cnt; i++) {
        struct bios_mmap_entry *entry = &memory_map[i];

        // Check if the memory region is of type MEMORY_TYPE_FREE
        if (entry->type == MEMORY_TYPE_FREE) {
            // Calculate the end address of this memory region
            uint64_t end_addr = entry->base_addr + entry->addr_len;

            // Update max_physical_address if needed
            if (end_addr > max_physical_address) {
                max_physical_address = end_addr;
            }

            // Count the number of pages in this memory region
            pages_cnt += (max_physical_address) / PAGE_SIZE;
            
            // pages_cnt += (end_addr - entry->base_addr) / PAGE_SIZE;
        }
    }
    // uint64_t page_cnt= ROUND_DOWN(max_physical_address, PAGE_SIZE/sizeof(struct page)) / PAGE_SIZE;
    // Round up max_physical_address to the nearest page boundary
    // Align pages_cnt using ROUND_DOWN
    max_physical_address = ROUND_UP(max_physical_address, PAGE_SIZE);
    pages_cnt = ROUND_DOWN(pages_cnt, PAGE_SIZE / sizeof(struct page));
...
int loader_init_memory(struct bios_mmap_entry *mm, uint32_t cnt, uint64_t pages_cnt)
{
    ...
    //SS Iterate through memory map entries to fill in the free pages list
    for (uint32_t i = 0; i < pages_cnt; i++)
    {
        uint64_t page_addr = (uint64_t)i * PAGE_SIZE;

        if (!page_is_available(page_addr, mm, cnt))
        {
            pages[i].ref = 1;
            continue;
        }

        //SS Insert the page at the head of the free list
        LIST_INSERT_HEAD(&state.free, &pages[i], link);
    }
